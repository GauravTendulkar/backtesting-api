...
def create_timeframe_files(duckdb_conn, stock, timeframe):
    ...
    if type(timeframe) == int and 1 <= timeframe and timeframe < 60*4:
        ...
        raw_data_path = f"Clean_data/1min/{stock}_1min.{RAW_FILE_EXTENSION}"
        if os.path.isfile(raw_data_path):
            ...
            FROM "Clean_data/1min/{stock}_1min.{RAW_FILE_EXTENSION}"
        ...
        # pass
        raw_data_path = f"Clean_data/RAW_daily_data_tradingview/{stock}_Daily.{RAW_FILE_EXTENSION}"
        if os.path.isfile(raw_data_path):
            # os.makedirs(f"{INDICATOR_FILE_DATABASE}/symbol={stock}/tf={TIMEFRAME_CONVERSION[timeframe]}", exist_ok=True)
...
def create_indicators(conn , create_indicator_list):
        ...
        print("indicator_to_be_created_list" , len(indicator_to_be_created_list) , indicator_to_be_created_list)

...
FAST_CACHE_FILE_DATABASE = "fastCache_duckdb"
                ...
                if if_index_exist(indicator_arr, [2]) and (indicator_arr[2] == "sma" 
                    ...
                    or indicator_arr[2] == "datetime_number" ):
                        for l in list_stocks:
                            ...
                            arr.append(f"{indicator_arr[1]}_{indicator_arr[0]}_{indicator_shortform}")
...
class BacktestingTable:
    ...
    def __init__(self, stock_list = [], duckdb_conn = duckdb.connect()):
        ...
        self.missing_candles_list_group_by_stock = {}
...
"""
    ...
    def get_raw_tf_data(self, time_frame, stock):
        final_text = f""" 
    SELECT
        {self.timeframe_columns(TIMEFRAME_CONVERSION_REVERSED[time_frame])}
        
    FROM read_parquet( 'indicator_process_duckdb/symbol={stock}/tf={time_frame}/data.parquet' )
    ...
    def first_Candle(self, smallest, candle, indicator, stock, tf, with_condition, start_date, end_date):
        ...
        if with_condition == True :
            def inner(n, indicator, stock, tf):
                ...
                numbered AS (
                    ...
                    FROM "{TIMEFRAME_CONVERSION[tf]}_table"
...
"""
    ...
    def accumulate_prepare_columns_for_each_stock(self, start_date , end_date, stock):
        length_of_stock_list = len([stock])
...
{self.prepare_columns_for_each_stock(stock , start_date , end_date)}
    ...
    def fetch_all_timeframes(self, stock):
...
{self.get_raw_tf_data(TIMEFRAME_CONVERSION[tf], stock)}
)"""
            final_text += text
            final_text = final_text + ",\n"
            final_text = str(final_text)
    ...
    def union_all_stock(self, stock):
        # length_of_stock_list = len(self.unique_stock) 
        length_of_stock_list = len([stock]) 
...
# for index, stock in enumerate(self.unique_stock):
    ...
    def create_fast_cache_columns(self, stock):
        final_text = f""" 
    
    WITH 
    {self.fetch_all_timeframes(stock)}
    {self.accumulate_prepare_columns_for_each_stock("2020-01-01"  , "2025-01-01", stock)}


    {self.union_all_stock(stock)}

    """
        
        return self.duckdb_conn.execute(final_text).df()
        
        

    def create_fast_cache(self):
        for stock in self.raw_data_group_by_stock.keys():
            file_path = f"{FAST_CACHE_FILE_DATABASE}/symbol={stock}/tf={TIMEFRAME_CONVERSION[self.smallest_tf]}/data.parquet"
            folder_path = f"{FAST_CACHE_FILE_DATABASE}/symbol={stock}/tf={TIMEFRAME_CONVERSION[self.smallest_tf]}"
            os.makedirs(folder_path, exist_ok=True)

            if os.path.isfile(file_path):
                print("fast Cache file exist")
                pass
            else:
                print("Create fast Cache file")
                self.missing_candles_list_group_by_stock[stock] = self.raw_data_group_by_stock[stock]
                df = self.create_fast_cache_columns(stock)
                query = f""" 
COPY (
SELECT * FROM df
)

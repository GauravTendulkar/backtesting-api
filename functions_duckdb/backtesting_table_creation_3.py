...
"""
    ...
    FROM read_parquet('indicator_process_duckdb/*/*/*.parquet' , hive_partitioning = true , union_by_name=true)
    WHERE symbol IN (
        ...
        if with_condition == True :
            def inner(n, indicator, stock, tf):
                ...
                numbered AS (
                    ...
                    FROM "{stock}_{TIMEFRAME_CONVERSION[tf]}"
    ...
    def previous_candle(self, smallest, candle, indicator, stock, tf, with_condition, start_date, end_date):
        ...
        if with_condition == True :
            def inner(n, indicator, stock, tf):
                ...
                FROM
                "{stock}_{TIMEFRAME_CONVERSION[tf]}"
                WHERE datetime BETWEEN '{start_date}' AND '{end_date}'
                --AND symbol = '{stock}'
                """
                return text
...
FROM "{stock}_{TIMEFRAME_CONVERSION[self.smallest_tf]}"
--WHERE symbol = '{stock}'

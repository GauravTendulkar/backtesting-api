...
def read_file(path, extension = extension, col= None):
    ...
    if extension == ".csv":
        if col is None:
            return pd.read_csv(f"{path}.csv",low_memory=False,parse_dates=['datetime'],index_col=['datetime'], usecols=col)
        elif "datetime" in col:
            return pd.read_csv(f"{path}.csv",low_memory=False,parse_dates=['datetime'],index_col=['datetime'], usecols=col)
        else:
            return pd.read_csv(f"{path}.csv",low_memory=False,parse_dates=['datetime'],index_col=['datetime'], usecols=col + ["datetime"])
    ...
    if extension == ".parquet":
        if col is None:
            ...
            return pd.read_parquet(f"{path}.parquet", columns=col + ["datetime"])
    ...
    if extension == ".feather":
        if col is None:
            ...
            return pd.read_feather(f"{path}.feather", columns=col + ["datetime"])

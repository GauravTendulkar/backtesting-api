...
from ..database.mongodb.repositories import strategy_run_collection, equation_collection
...
async def get_trending_strategies(
    ...
    trending_links = strategy_run_collection.get_agg_pipeline(pipeline)
    ...
    #     }
    # ))
    content_docs = equation_collection.get_link_info_with_link_isprivate(links)
...
async def get_top_liked_strategies(tag: str = Query(...)):
    ...
    documents = equation_collection.get_top_20_liked_content(query, projection)
...
def search_strategies(
    ...
    search_result = equation_collection.get_search_Result_with_pagenation(filters, skip, page_size)
    cursor = search_result["search_result"]

...
from .config import CLEAN_DATA_1MIN, CLEAN_DATA_DAILY, CLEAN_NEW_DATA
...
def check_stock_files_if_exists(stocks_list):
    ...
    files = [f for f in os.listdir(CLEAN_DATA_1MIN) if os.path.isfile(os.path.join(CLEAN_DATA_1MIN, f))]
    try:
    ...
    temp_Daily = []
    files = [f for f in os.listdir(CLEAN_DATA_DAILY) if os.path.isfile(os.path.join(CLEAN_DATA_DAILY, f))]
    try:
        files.remove('.gitkeep')
    except:
        pass

...
def convert_df_to_json_and_maxstocks(df):
    ...
    # 🔥 Create dict per row before grouping
    df_true = df_true.copy()
    df_true['stock_info'] = df_true.apply(
        lambda x: {
            "stock": x['stock'],
    ...
    # Group by timestamp
    grouped = df_true.groupby('entryDateTime')['stock_info'].agg(list).sort_index()
...
def duckdb_condition_scanner_4(data, date_ranges,stock_list, stock_list_details, create, duckdb_conn):
...
#__________________________________________________________
    ...
    for f in range(0, len(final_text_array)):
        ...
        result["company_name"] = np.empty(initial_length, dtype=object)
        ...
        for i in range(0, len(df["datetime"])):
            ...
            try:
                if eval(pre_c_entry, globals(), locals()):
                    ...
                    result["stock"][i] = stock_list_details[df["symbol"][i]]["symbol"]
                    result["company_name"][i] = stock_list_details[df["symbol"][i]]["company_name"]
                ...
                result["condition"][i] = False
                result["entryDateTime"][i] = s
        Tracking = pd.concat([Tracking, pd.DataFrame(result)],  ignore_index=True)

...
# Exit Exit Exit Exit Exit Exit Exit Exit ________________________________________________
                ...
                for exitCollection_index in range(len(exitCollection)):
                    ...
                    if eval(pre_c_exitCollection[exitCollection_index]["exit"], globals(), locals()):
                        Tracking.loc[Tracking.index[-1], 'exit'] = eval(pre_c_exitCollection[exitCollection_index]["exitPrice"], globals(), locals())  #float(Tracking.loc[Tracking.index[-1], 'entry'] *(1-0.015*1))
                        ...
                        Tracking.loc[Tracking.index[-1], 'exit_date'] = f'{dn[0:4]}-{dn[4:6]}-{dn[6:8]}'
                        flag_exit = 0
                        flag_entry = 0
                        # print("break")
                        dict_cache = {
...
def long_running_6(data):
    print("long_running_6")

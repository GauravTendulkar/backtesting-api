...
# do not enter if candle is last for intraday
            ...
            if flag_entry ==  1 :
                ...
                try:
                    ...
                    # }   
                        # print("eval(pre_c_entryPrice, globals(), locals())", eval(pre_c_entryPrice, globals(), locals()))
                        # print(i, str(buysell))
                        ...
                        entry_array[Tracking_index] =     eval(pre_c_entryPrice, globals(), locals())
                        entryTime_array[Tracking_index] = f'{trading_time[-6:-4]}:{trading_time[-4:-2]}'
                        # exit_date_array[Tracking_index] = ""
                        # exit_array[Tracking_index] = 0
                        # exitTime_array[Tracking_index] = ''
                        quantity_array[Tracking_index] =  round( eval(pre_c_quantity, globals(), locals()) ,0) 
                        # pnl_array[Tracking_index] = 0
                        # sl_array[Tracking_index] = ''
                        date_number_array[Tracking_index] = int(df_data[i][date_index])
...
# Exit Exit Exit Exit Exit Exit Exit Exit ________________________________________________
                ...
                for exitCollection_index in range(len(exitCollection)):
                    ...
                    if eval(pre_c_exitCollection[exitCollection_index]["exit"], globals(), locals()):
                        trading_time = str(int(df_data[i][time_index]))
                        trading_date = str(df_data[i][date_index])
                        exit_array[Tracking_index] = eval(pre_c_exitCollection[exitCollection_index]["exitPrice"], globals(), locals())  #float(Tracking.loc[Tracking.index[-1], 'entry'] *(1-0.015*1))
                        exitTime_array[Tracking_index] = f'{trading_time[-6:-4]}:{trading_time[-4:-2]}'
                        sl_array[Tracking_index] = exitCollection[exitCollection_index]["label"]
    ...
    # universal exit
                if smallest != "Daily" and smallest != "Weekly" and smallest != "Monthly" and tradeSetup != "long" and tradeSetup != "short":
                    ...
                    if int(trading_time_temp) == df_time and flag_exit == 1  :
                        ...
                        exit_array[Tracking_index] = float(df_fastCache_colum_arr[i][fastCache_colum_name.index(smallest_close)]  )
    ...
    Tracking = pd.DataFrame({
    'stock': stock_array,
    'buysell': buysell_array,
    'date': date_array,
    'entryTime': entryTime_array,
    'entry': entry_array,
    'exit_date': exit_date_array,
    'exitTime': exitTime_array,
    'exit': exit_array,
...
def long_running_13(data, date_ranges):
    print("long_running_13")
